package com.tcdt.qlnvhang.entities.nhaphang.nhapkhac.kiemtracl.bblaymaubangiaomau;
...
@Entity
@Table(name = "NH_BB_LAY_MAU_KHAC")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class BienBanLayMauKhac extends TrangThaiBaseEntity implements Serializable  {
	private static final long serialVersionUID = 6093365068005372524L;
	public static final String TABLE_NAME = "NH_BB_LAY_MAU_KHAC";
	@Id
...
//	@SequenceGenerator(sequenceName = "BB_LAY_MAU_SEQ", allocationSize = 1, name = "BB_LAY_MAU_SEQ")
	...
	@Transient
	private List<BienBanLayMauKhacCt> chiTiets = new ArrayList<>();
	...
	@Where(clause = "data_type='" + BienBanLayMauKhac.TABLE_NAME + "'")
	private List<FileDKemJoinBbLayMauKhac> fileDinhKems = new ArrayList<>();

	public void setFileDinhKems(List<FileDKemJoinBbLayMauKhac> children) {
		this.fileDinhKems.clear();
		for (FileDKemJoinBbLayMauKhac child : children) {
			child.setParent(this);
		}
		this.fileDinhKems.addAll(children);
	}

	public void addFileDinhKems(FileDKemJoinBbLayMauKhac child) {
		child.setParent(this);
