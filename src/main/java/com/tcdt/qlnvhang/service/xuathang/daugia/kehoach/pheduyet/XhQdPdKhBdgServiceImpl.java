...
public class XhQdPdKhBdgServiceImpl extends BaseServiceImpl {
  ...
  public List<XhQdPdKhBdgDtl> detailDtl(List<Long> ids, CustomUserDetails currentUser) throws Exception {
    if (DataUtils.isNullOrEmpty(ids)) {
      throw new Exception("Tham số không hợp lệ.");
    }
    ...
    for (XhQdPdKhBdgDtl item : resultList) {
      ...
      if (!currentUser.getUser().getCapDvi().equals(Contains.CAP_TONG_CUC)) {
        biddingInfoList = biddingInfoList.stream().filter(s -> s.getMaDvi().contains(currentUser.getDvql())).collect(Collectors.toList());
      }
