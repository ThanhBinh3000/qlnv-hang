...
public class XhHopDongServiceImpl extends BaseServiceImpl {
  ...
  @Transactional
  public XhHopDongHdr preCreate(CustomUserDetails currentUser, XhHopDongHdrReq request) throws Exception {
    if (currentUser == null || request == null) {
      throw new Exception("Bad request.");
    }
    if (!StringUtils.isEmpty(request.getSoHopDong()) && DataUtils.isNullObject(request.getIdHopDong()) && xhHopDongHdrRepository.existsBySoHopDong(request.getSoHopDong())) {
    ...
    /////////
    XhHopDongHdr xhHopDongHdr = new XhHopDongHdr();
    if (!DataUtils.isNullObject(request.getXhKqBdgHdr())) {
      XhKqBdgHdr xhKqBdgBdg = xhKqBdgHdrRepository.findFirstBySoQdKq(request.getXhKqBdgHdr().getSoQdPd()).orElse(new XhKqBdgHdr());
      if (DataUtils.isNullObject(xhKqBdgBdg.getId())) {
        xhKqBdgBdg.setSoQdKq(request.getXhKqBdgHdr().getSoQdKq());
        xhKqBdgBdg.setNam(request.getXhKqBdgHdr().getNam());
        xhKqBdgBdg.setMaDvi(currentUser.getDvql().substring(0, 6));
        xhKqBdgBdg.setLoaiVthh(request.getLoaiVthh());
        xhKqBdgBdg.setCloaiVthh(request.getCloaiVthh());
        xhKqBdgBdg.setTrangThai(Contains.BAN_HANH);
        xhKqBdgBdg.setTrangThaiNhapLieu(Contains.TRANG_THAI_NHAP_LIEU.DANG_NHAP);
        xhKqBdgBdg = xhKqBdgHdrRepository.save(xhKqBdgBdg);
          /*BeanUtils.copyProperties(request, xhHopDongHdr);
          xhHopDongHdr.setIdQdKq(xhKqBdgBdg.getId());
          xhHopDongHdr.setMaDvi(currentUser.getDvql());
          xhHopDongHdr.setTrangThaiNhapLieu(Contains.TRANG_THAI_NHAP_LIEU.DANG_NHAP);
          xhHopDongHdr.setTrangThai(Contains.DUTHAO);
          xhHopDongHdr = xhHopDongHdrRepository.save(xhHopDongHdr);*/
      }
    }
  ...
  @Transactional
  public XhHopDongHdr preUpdate(CustomUserDetails currentUser, XhHopDongHdrReq request) throws Exception {
    if (currentUser == null || request == null || request.getId() == null) {
      throw new Exception("Bad request.");
    }
    if (DataUtils.isNullObject(request.getXhKqBdgHdr())) {
    ...
    XhKqBdgHdr oldXhKqBdgHdr = xhKqBdgHdrRepository.findById(request.getXhKqBdgHdr().getId())
        .orElseThrow(() -> new Exception("Không tìm thấy dữ liệu cần sửa"));
    XhHopDongHdr existingData = xhHopDongHdrRepository.findById(request.getId())
        .orElseThrow(() -> new Exception("Không tìm thấy dữ liệu cần sửa"));

    BeanUtils.copyProperties(request.getXhKqBdgHdr(), oldXhKqBdgHdr, "id", "maDvi");
    xhKqBdgHdrRepository.save(oldXhKqBdgHdr);
  ...
  @Transactional
  public XhKqBdgHdr preApprove(CustomUserDetails currentUser, StatusReq statusReq) throws Exception {
    if (currentUser == null || StringUtils.isEmpty(statusReq.getId())) {
      throw new Exception("Bad request.");
    }
    XhKqBdgHdr oldXhKqBdgHdr = xhKqBdgHdrRepository.findById(statusReq.getId())
        .orElseThrow(() -> new Exception("Không tìm thấy dữ liệu"));
    if (DataUtils.isNullOrEmpty(statusReq.getTrangThai())) {
      statusReq.setTrangThai(Contains.TRANG_THAI_NHAP_LIEU.HOAN_THANH);
    }
    String strStatus = oldXhKqBdgHdr.getTrangThaiNhapLieu() + statusReq.getTrangThai();
    switch (strStatus) {
      case Contains.TRANG_THAI_NHAP_LIEU.DANG_NHAP + Contains.TRANG_THAI_NHAP_LIEU.HOAN_THANH:
        oldXhKqBdgHdr.setTrangThaiNhapLieu(Contains.TRANG_THAI_NHAP_LIEU.HOAN_THANH);
        break;
      default:
        throw new Exception("Hoàn thành trạng thái không thành công");
    }
    XhKqBdgHdr updatedData = xhKqBdgHdrRepository.save(oldXhKqBdgHdr);
    return updatedData;
  }
}