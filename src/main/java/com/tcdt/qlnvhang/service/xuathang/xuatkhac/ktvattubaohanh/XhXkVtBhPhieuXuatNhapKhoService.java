    ...
    if (!DataUtils.isNullObject(phieuXuatNhapKho.getIdCanCu())) {
      ...
      if (phieuXuatKho.isPresent()) {
        ...
        for (XhXkVtBhQdGiaonvXnDtl f : qdGiaonvXhDtl) {
          if (f.getMaDiaDiem().equals(phieuXuatNhapKho.getMaDiaDiem())) {
            if (item.getLoai().equals("XUAT")) {
              f.setTrangThaiXh(xoa ? TrangThaiAllEnum.DANG_THUC_HIEN.getId() : null);
            } else {
              f.setTrangThaiXh(xoa ? TrangThaiAllEnum.DA_HOAN_THANH.getId() : null);
            }
          }
        }
