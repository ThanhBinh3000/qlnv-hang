...
@Log4j2
@Service
public class XhBangKeCanHangServiceImpl extends BaseServiceImpl implements XhBangKeCanHangService {

    private static final String SHEET_BANG_KE_CAN_HANG = "Bảng kê cân hàng";

    private final String MA_DS = "/BKCH-CCDTVP";
    @Autowired
    XhBangKeCanHangRepository mainRepository;

    @Autowired
    XhBangKeCanHangCtRepository subRepository;

    @Override
    public Page<XhBangKeCanHang> searchPage(XhBangKeCanHangReq req) throws Exception {
        Pageable pageable = PageRequest.of(req.getPaggingReq().getPage(),
                req.getPaggingReq().getLimit(), Sort.by("id").descending());
        Page<XhBangKeCanHang> data = mainRepository.searchPage( req,pageable);
        return data;
    }

    @Override
    public XhBangKeCanHang create(XhBangKeCanHangReq req) throws Exception {
        UserInfo userInfo = SecurityContextService.getUser();
        if (userInfo == null) {
            throw new Exception("Bad request.");
        }

        XhBangKeCanHang data = new XhBangKeCanHang();
...
//        }
    ...
    @Override
    public XhBangKeCanHang update(XhBangKeCanHangReq req) throws Exception {
        ...
        Optional<XhBangKeCanHang> byId = mainRepository.findById(req.getId());
        if(!byId.isPresent()){
            throw new Exception("Không tìm thấy dữ liệu");
        }
        XhBangKeCanHang dataDb = byId.get();
...
//        saveDetail(req,data.getId());
    ...
    @Override
    public XhBangKeCanHang detail(Long id) throws Exception {
        if(Objects.isNull(id)){
            throw new Exception("Bad reqeust");
        }

        Optional<XhBangKeCanHang> byId = mainRepository.findById(id);
        if(!byId.isPresent()){
            throw new Exception("Không tìm thấy dữ liệu");
        }
        Map<String, String> mapDmucHh = getListDanhMucHangHoa();
        Map<String, String> mapDmucDvi = getListDanhMucDvi(null, null, "01");
        XhBangKeCanHang data = byId.get();
...
//        data.setChildren(subRepository.findAllByIdHdr(id));
    ...
    @Override
    public XhBangKeCanHang approve(XhBangKeCanHangReq req) throws Exception {
    ...
    @Override
    public void export(XhBangKeCanHangReq req, HttpServletResponse response) throws Exception {
