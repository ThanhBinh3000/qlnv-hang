...
public class XhQdGiaoNvuXuatServiceImpl extends BaseServiceImpl implements XhQdGiaoNvuXuatService {
    ...
    private List<XhQdGiaoNvuXuatCt1> saveListChiTiet1(Long parentId, List<Long> hopDongIds) {
        xhQdGiaoNvuXuatCt1Repository.deleteByQdgnvxIdIn(Collections.singleton(parentId));
        ...
        if (!CollectionUtils.isEmpty(ct1s))
            xhQdGiaoNvuXuatCt1Repository.saveAll(ct1s);
        return ct1s;
    }
    ...
    private List<XhQdGiaoNvuXuatCt> saveListChiTiet(Long parentId,
        ...
        if (!CollectionUtils.isEmpty(chiTiets))
            xhQdGiaoNvuXuatCtRepository.saveAll(chiTiets);
    ...
    public XhQdGiaoNvuXuatRes update(XhQdGiaoNvuXuatReq req) throws Exception {
        ...
        Optional<XhQdGiaoNvuXuat> optional = xhQdGiaoNvuXuatRepository.findById(req.getId());
    ...
    public XhQdGiaoNvuXuatRes detail(Long id) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        Optional<XhQdGiaoNvuXuat> optional = xhQdGiaoNvuXuatRepository.findById(id);
        if (!optional.isPresent())
            throw new Exception("Quyết định giao nhiệm vụ xuất không tồn tại.");

        XhQdGiaoNvuXuat item = optional.get();
        item.setCts(xhQdGiaoNvuXuatCtRepository.findByQdgnvxIdIn(Collections.singleton(item.getId())));
        item.setCt1s(xhQdGiaoNvuXuatCt1Repository.findByQdgnvxIdIn(Collections.singleton(item.getId())));
    ...
    public boolean delete(Long id) throws Exception {
        ...
        xhQdGiaoNvuXuatCtRepository.deleteByQdgnvxIdIn(Collections.singleton(item.getId()));
        xhQdGiaoNvuXuatCt1Repository.deleteByQdgnvxIdIn(Collections.singleton(item.getId()));
        xhQdGiaoNvuXuatRepository.delete(item);
        return true;
    }
    ...
    public boolean updateStatusQd(StatusReq stReq) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        Optional<XhQdGiaoNvuXuat> optional = xhQdGiaoNvuXuatRepository.findById(stReq.getId());
        if (!optional.isPresent())
            throw new Exception("Quyết định giao nhiệm vụ xuất không tồn tại.");
    ...
    public boolean deleteMultiple(DeleteReq req) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        xhQdGiaoNvuXuatCtRepository.deleteByQdgnvxIdIn(req.getIds());
        xhQdGiaoNvuXuatCt1Repository.deleteByQdgnvxIdIn(req.getIds());
        fileDinhKemService.deleteMultiple(req.getIds(), Collections.singleton(XhQdGiaoNvuXuat.TABLE_NAME));
        xhQdGiaoNvuXuatRepository.deleteByIdIn(req.getIds());
        return true;
    }
    ...
    public Integer getSo() throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        Integer so = xhQdGiaoNvuXuatRepository.findMaxSo(userInfo.getDvql(), LocalDate.now().getYear());
    ...
    private void validateSoQuyetDinh(XhQdGiaoNvuXuat update, XhQdGiaoNvuXuatReq req) throws Exception {
        ...
        if (update == null || (StringUtils.hasText(update.getSoQuyetDinh()) && !update.getSoQuyetDinh().equalsIgnoreCase(so))) {
            Optional<XhQdGiaoNvuXuat> optional = xhQdGiaoNvuXuatRepository.findFirstBySoQuyetDinh(so);
