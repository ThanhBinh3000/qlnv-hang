...
import com.tcdt.qlnvhang.request.object.vattu.phieunhapkho.NhPhieuNhapKhoVtCtReq;
...
import com.tcdt.qlnvhang.response.vattu.phieunhapkho.NhPhieuNhapKhoVtCtRes;
...
public class NhPhieuNhapKhoVtServiceImpl extends BaseServiceImpl implements NhPhieuNhapKhoVtService {
    private final NhPhieuNhapKhoVtRepository nhPhieuNhapKhoVtRepository;
    private final NhPhieuNhapKhoVtCtRepository phieuNhapKhoVtCtRepository;
    ...
    public NhPhieuNhapKhoVtRes create(NhPhieuNhapKhoVtReq req) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        this.validateSoPhieu(null, req);

        NhPhieuNhapKhoVt item = new NhPhieuNhapKhoVt();
        ...
        // TODO : mã phiếu
        item.setSoPhieu(String.format("%s/%s/%s", item.getSo(), item.getNam(), "PNKVT"));
        nhPhieuNhapKhoVtRepository.save(item);

        List<NhPhieuNhapKhoVtCt> chiTiets = this.saveListChiTiet(item.getId(), req.getChiTiets(), new HashMap<>());
        item.setChiTiets(chiTiets);
        List<FileDinhKem> fileDinhKems = fileDinhKemService.saveListFileDinhKem(req.getFileDinhKems(), item.getId(), NhPhieuNhapKhoVt.TABLE_NAME);
        item.setFileDinhKems(fileDinhKems);

        return this.buildResponse(item);
    }

    private List<NhPhieuNhapKhoVtCt> saveListChiTiet(Long parentId,
                                                     List<NhPhieuNhapKhoVtCtReq> chiTietReqs,
                                                     Map<Long, NhPhieuNhapKhoVtCt> mapChiTiet) throws Exception {
        List<NhPhieuNhapKhoVtCt> chiTiets = new ArrayList<>();
        for (NhPhieuNhapKhoVtCtReq req : chiTietReqs) {
            Long id = req.getId();
            NhPhieuNhapKhoVtCt chiTiet = new NhPhieuNhapKhoVtCt();
            ...
            chiTiet.setPhieuNkId(parentId);
            chiTiets.add(chiTiet);
        }

        if (!CollectionUtils.isEmpty(chiTiets))
            phieuNhapKhoVtCtRepository.saveAll(chiTiets);

        return chiTiets;
    }


    private NhPhieuNhapKhoVtRes buildResponse(NhPhieuNhapKhoVt item) throws Exception {
        NhPhieuNhapKhoVtRes res = new NhPhieuNhapKhoVtRes();
        List<NhPhieuNhapKhoVtCtRes> chiTiets = new ArrayList<>();
        ...
        for (NhPhieuNhapKhoVtCt ct : item.getChiTiets()) {
            chiTiets.add(new NhPhieuNhapKhoVtCtRes(ct));
        }
    ...
    @Transactional(rollbackOn = Exception.class)
    @Override
    public NhPhieuNhapKhoVtRes update(NhPhieuNhapKhoVtReq req) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();

        Optional<NhPhieuNhapKhoVt> optional = nhPhieuNhapKhoVtRepository.findById(req.getId());
        if (!optional.isPresent())
            throw new Exception("Phiếu nhập kho vật tư không tồn tại.");

        this.validateSoPhieu(optional.get(), req);

        NhPhieuNhapKhoVt item = optional.get();
        BeanUtils.copyProperties(req, item, "id", "so", "nam");
        item.setNgaySua(LocalDate.now());
        item.setNguoiSuaId(userInfo.getId());

        nhPhieuNhapKhoVtRepository.save(item);
        Map<Long, NhPhieuNhapKhoVtCt> mapChiTiet = phieuNhapKhoVtCtRepository.findByPhieuNkIdIn(Collections.singleton(item.getId()))
                .stream().collect(Collectors.toMap(NhPhieuNhapKhoVtCt::getId, Function.identity()));

        List<NhPhieuNhapKhoVtCt> chiTiets = this.saveListChiTiet(item.getId(), req.getChiTiets(), mapChiTiet);
        item.setChiTiets(chiTiets);
        if (!CollectionUtils.isEmpty(mapChiTiet.values()))
            phieuNhapKhoVtCtRepository.deleteAll(mapChiTiet.values());
    ...
    @Override
    public NhPhieuNhapKhoVtRes detail(Long id) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        Optional<NhPhieuNhapKhoVt> optional = nhPhieuNhapKhoVtRepository.findById(id);
        if (!optional.isPresent())
            throw new Exception("Phiếu nhập kho vật tư không tồn tại.");

        NhPhieuNhapKhoVt item = optional.get();
        item.setChiTiets(phieuNhapKhoVtCtRepository.findByPhieuNkIdIn(Collections.singleton(item.getId())));
        item.setFileDinhKems(fileDinhKemService.search(item.getId(), Collections.singleton(NhPhieuNhapKhoVt.TABLE_NAME)));
        return this.buildResponse(item);
    }
    ...
    public boolean delete(Long id) throws Exception {
        ...
        phieuNhapKhoVtCtRepository.deleteByPhieuNkIdIn(Collections.singleton(item.getId()));
        nhPhieuNhapKhoVtRepository.delete(item);
        return true;
    }
    ...
    public boolean updateStatusQd(StatusReq stReq) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        Optional<NhPhieuNhapKhoVt> optional = nhPhieuNhapKhoVtRepository.findById(stReq.getId());
        if (!optional.isPresent())
            throw new Exception("Phiếu nhập kho vật tư không tồn tại.");

        NhPhieuNhapKhoVt phieu = optional.get();
        ...
        nhPhieuNhapKhoVtRepository.save(phieu);
        return true;
    }

    @Override
    public Page<NhPhieuNhapKhoVtRes> search(NhPhieuNhapKhoVtSearchReq req) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();

        this.prepareSearchReq(req, userInfo, req.getCapDvis(), req.getTrangThais());
        Pageable pageable = PageRequest.of(req.getPaggingReq().getPage(), req.getPaggingReq().getLimit());
        List<Object[]> data = nhPhieuNhapKhoVtRepository.search(req);
        List<NhPhieuNhapKhoVtRes> responses = new ArrayList<>();
        for (Object[] o : data) {
            NhPhieuNhapKhoVtRes response = new NhPhieuNhapKhoVtRes();
            NhPhieuNhapKhoVt item = (NhPhieuNhapKhoVt) o[0];
        ...
        return new PageImpl<>(responses, pageable, nhPhieuNhapKhoVtRepository.count(req));
    }
    ...
    public boolean deleteMultiple(DeleteReq req) throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        phieuNhapKhoVtCtRepository.deleteByPhieuNkIdIn(req.getIds());
        nhPhieuNhapKhoVtRepository.deleteByIdIn(req.getIds());
        return true;
    }

    @Override
    public boolean exportToExcel(NhPhieuNhapKhoVtSearchReq objReq, HttpServletResponse response) throws Exception {

        UserInfo userInfo = UserUtils.getUserInfo();
        this.prepareSearchReq(objReq, userInfo, objReq.getCapDvis(), objReq.getTrangThais());
        objReq.setPaggingReq(new PaggingReq(Integer.MAX_VALUE, 0));
        List<NhPhieuNhapKhoVtRes> list = this.search(objReq).get().collect(Collectors.toList());
        ...
        try {
            ...
            XSSFSheet sheet = workbook.createSheet(SHEET_PHIEU_NHAP_KHO_VAT_TU);
            ...
            for (NhPhieuNhapKhoVtRes item : list) {
    ...
    private void thongTinNganLo(NhPhieuNhapKhoVtRes item, KtNganLo nganLo) {
        if (nganLo != null) {
    ...
    private void validateSoPhieu(NhPhieuNhapKhoVt update, NhPhieuNhapKhoVtReq req) throws Exception {
        ...
        if (update == null || (StringUtils.hasText(update.getSoPhieu()) && !update.getSoPhieu().equalsIgnoreCase(so))) {
            Optional<NhPhieuNhapKhoVt> optional = nhPhieuNhapKhoVtRepository.findFirstBySoPhieu(so);
            Long updateId = Optional.ofNullable(update).map(NhPhieuNhapKhoVt::getId).orElse(null);
    ...
    public Integer getSo() throws Exception {
        UserInfo userInfo = UserUtils.getUserInfo();
        Integer so = nhPhieuNhapKhoVtRepository.findMaxSo(userInfo.getDvql(), LocalDate.now().getYear());
