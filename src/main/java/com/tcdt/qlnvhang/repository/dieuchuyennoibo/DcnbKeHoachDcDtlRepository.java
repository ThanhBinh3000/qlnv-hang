...
public interface DcnbKeHoachDcDtlRepository extends JpaRepository<DcnbKeHoachDcDtl,Long> {
    ...
    @Query(nativeQuery = true,value ="SELECT SUM(d.DU_TOAN_KPHI) FROM DCNB_KE_HOACH_DC_DTL d LEFT JOIN DCNB_KE_HOACH_DC_HDR h ON h.ID = d.HDR_ID WHERE h.MA_DVI = ?1 AND h.MA_DVI_CUC = ?2 AND h.MA_CUC_NHAN = ?3 AND h.TYPE = 'DC' AND h.LOAI_DC = 'CUC' AND h.TRANG_THAI = '17' AND (TO_DATE(TO_CHAR(h.NGAY_TAO,'YYYY-MM-DD'),'YYYY-MM-DD') <= TO_DATE(?4,'YYYY-MM-DD'))")
