...
public interface DcnbQuyetDinhDcTcHdrRepository extends JpaRepository<DcnbQuyetDinhDcTcHdr, Long> {
    ...
    @Query(value = "SELECT distinct * FROM DCNB_QUYET_DINH_DC_TC_HDR hdr WHERE 1=1 " +
            ...
            "AND EXISTS(SELECT * FROM DCNB_QUYET_DINH_DC_TC_DTL dtl JOIN DCNB_KE_HOACH_DC_HDR dchdr ON dtl.DCNB_KE_HOACH_DC_HDR_ID = dchdr.ID WHERE (dchdr.MA_DVI_CUC = CONCAT(:#{#param.maDvi},'') OR dchdr.MA_CUC_NHAN = CONCAT(:#{#param.maDvi},''))) " +
