...
public interface DcnbPhieuKtChatLuongHdrRepository extends JpaRepository<DcnbPhieuKtChatLuongHdr, Long> {
    ...
    @Query(value = "SELECT new com.tcdt.qlnvhang.response.dieuChuyenNoiBo.DcnbPhieuKtChatLuongHdrDTO(" +
            "pkncl.id,bblm.id,qdc.id,qdc.soQdinh,bblm.soBbLayMau,bblm.ngayLayMau,qdc.nam,khdcd.thoiGianDkDc,khdcd.maNhaKho,khdcd.tenNhaKho,khdcd.maDiemKho,khdcd.tenDiemKho,khdcd.maLoKho," +
            "khdcd.tenLoKho,khdcd.maNganKho,khdcd.tenNganKho, khdcd.thayDoiThuKho,pkncl.soPhieu,pkncl.ngayKiem,pkncl.ngayKiem, pkncl.trangThai,pkncl.trangThai) FROM DcnbQuyetDinhDcCHdr qdc " +
