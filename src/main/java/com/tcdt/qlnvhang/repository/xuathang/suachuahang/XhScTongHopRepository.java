...
public interface XhScTongHopRepository extends JpaRepository<XhScTongHopHdr, Long> {
  @Query("SELECT distinct c FROM XhScTongHopHdr c left join c.tongHopDtl h WHERE 1=1 " +
  ...
  Page<XhScTongHopHdr> searchPage(@Param("param") XhTlTongHopRequest param, Pageable pageable);


  void deleteAllByIdIn(List<Long> listId);

  List<XhScTongHopHdr> findByIdIn(List<Long> ids);

  Optional<XhScTongHopHdr> findByMaDanhSach(String maDs);
}
