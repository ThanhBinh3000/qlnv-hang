...
@Slf4j
@RestController
@RequestMapping(value = PathContains.KNGHIEM_CLUONG_KHAC)
@Api(tags = "Nhập hàng - Đấu thầu - Kiểm tra chất lượng - Lập và ký phiếu kiểm nghiệm chất lượng")
public class NhPhieuKnghiemCluongHangKhacController {
	@Autowired
	private PhieuKnghiemCluongHangKhacService phieuKnghiemCluongHangService;

	@ApiOperation(value = "Tạo mới ", response = BienBanLayMauRes.class)
	@PostMapping(value = PathContains.URL_TAO_MOI, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<BaseResponse> create(@Valid @RequestBody PhieuKnghiemCluongHangKhacReq req) {
		BaseResponse resp = new BaseResponse();
		try {
			PhieuKnghiemCluongHangKhac res = phieuKnghiemCluongHangService.create(req);
			resp.setData(res);
	...
	public ResponseEntity<BaseResponse> update(@Valid @RequestBody PhieuKnghiemCluongHangKhacReq req) {
		BaseResponse resp = new BaseResponse();
		try {
			PhieuKnghiemCluongHangKhac res = phieuKnghiemCluongHangService.update(req);
			resp.setData(res);
			resp.setStatusCode(EnumResponse.RESP_SUCC.getValue());
			resp.setMsg(EnumResponse.RESP_SUCC.getDescription());
		} catch (Exception e) {
	...
	public ResponseEntity<BaseResponse> delete(@RequestBody PhieuKnghiemCluongHangKhacReq req) {
		BaseResponse resp = new BaseResponse();
		try {
	...
	public ResponseEntity<BaseResponse> updateStatus(@Valid @RequestBody PhieuKnghiemCluongHangKhacReq req) {
		BaseResponse resp = new BaseResponse();
		try {
...
//			phieuKnghiemCluongHangService.exportToExcel(req, response);
	...
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<BaseResponse> preview(@RequestBody PhieuKnghiemCluongHangSearchReq req) {
		BaseResponse resp = new BaseResponse();
		try {
			resp.setData(phieuKnghiemCluongHangService.preview(req));
			resp.setStatusCode(EnumResponse.RESP_SUCC.getValue());
			resp.setMsg(EnumResponse.RESP_SUCC.getDescription());
		} catch (Exception e) {
			resp.setStatusCode(EnumResponse.RESP_FAIL.getValue());
