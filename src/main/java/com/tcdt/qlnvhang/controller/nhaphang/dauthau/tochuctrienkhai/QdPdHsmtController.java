package com.tcdt.qlnvhang.controller.nhaphang.dauthau.tochuctrienkhai;
...
import com.tcdt.qlnvhang.service.nhaphang.dauthau.tochuctrienkhai.QdPdHsmtService;
...
@RequestMapping(value = PathContains.DAU_THAU_VAT_TU + PathContains.QUYET_DINH_PD_HS_MOI_THAU)
@Api(tags = "Nhập hàng - Đấu thầu - Tổ chức triển khai KH LCNT - Quyết định phê duyệt hồ sơ mời thầu ")
public class QdPdHsmtController {
    @Autowired
    private QdPdHsmtService service;

    @ApiOperation(value = "Tạo mới", response = List.class)
    @PostMapping(value = PathContains.URL_TAO_MOI, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.CREATED)
    public ResponseEntity<BaseResponse> insert(@Valid HttpServletRequest request,
                                               @RequestBody QdPdHsmtReq objReq) {
    ...
    public ResponseEntity<BaseResponse> selectAll(@Valid HttpServletRequest request,
                                                  @RequestBody QdPdHsmtSearchReq objReq) {
        BaseResponse resp = new BaseResponse();
        try {
