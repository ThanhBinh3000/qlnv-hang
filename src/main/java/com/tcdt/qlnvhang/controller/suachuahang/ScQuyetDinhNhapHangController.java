...
import com.tcdt.qlnvhang.service.suachuahang.impl.ScQuyetDinhNhapHangImpl;
...
@RestController
@RequestMapping(value = PathContains.SUA_CHUA + PathContains.NHAP_HANG)
@Slf4j
@Api(tags = "Sửa chữa hàng DTQG - Kiểm tra chất lượng sau SC")
public class ScQuyetDinhNhapHangController {
    @Autowired
    ScQuyetDinhNhapHangImpl service;
    ...
    @ApiOperation(value = "Tra cứu thông tin biên bản", response = List.class)
    @PostMapping(value = "ds-phieu-nhap-kho", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<BaseResponse> getDsPhieuNhapKho(@RequestBody ScQuyetDinhNhapHangReq objReq) {
        BaseResponse resp = new BaseResponse();
        try {
            resp.setData(service.dsTaoPhieuNhapKho(objReq));
            resp.setStatusCode(EnumResponse.RESP_SUCC.getValue());
            resp.setMsg(EnumResponse.RESP_SUCC.getDescription());
        } catch (Exception e) {
            e.printStackTrace();
