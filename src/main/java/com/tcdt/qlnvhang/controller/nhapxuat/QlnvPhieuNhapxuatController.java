...
public class QlnvPhieuNhapxuatController extends BaseController {
	...
	public ResponseEntity<BaseResponse> approve(HttpServletRequest req, @Valid @RequestBody StatusReq stReq) {
		...
		try {
			...
			if (qHoach.get().getTrangThai().equals(Contains.DUYET))
				throw new Exception("Dữ liệu đã được phê duyệt");
			...
			if (status.equals(Contains.DUYET))
	...
	public void export(@Valid @RequestBody IdSearchReq searchReq, HttpServletResponse response, HttpServletRequest req)
		...
		try {
			...
			mappings.put("param1", getDvi(req).getTenDvi());
			mappings.put("param2", "");
			mappings.put("param3", getDateText(qPhieu.get().getNgayLap()));
			mappings.put("param4", qPhieu.get().getSoPhieu());
			mappings.put("param5", qPhieu.get().getTkhoanNo());
			mappings.put("param6", qPhieu.get().getTkhoanCo());
			mappings.put("param7", "");
			mappings.put("param8", "");
			mappings.put("param9", "Hợp đồng");
			mappings.put("param10", qPhieu.get().getSoQdinhNhapxuat());
			mappings.put("param11", "");
			mappings.put("param12", "");
			mappings.put("param13", "");
			mappings.put("param14", "");
			mappings.put("param15", "");
			mappings.put("param16", MoneyConvert.docSoLuong(soLuong.toString(), detailList.get(0).getDviTinh()));
			mappings.put("param17", MoneyConvert.doctienBangChu(tongTien.toString(), null));
			mappings.put("param18", "");
			mappings.put("param19", getDateText(getDateTimeNow()));
