...
import com.tcdt.qlnvhang.service.xuathang.xuatcuutrovientroxuatcap.xuatcuutrovientro.XhCtvtPhieuKnClHdrService;
...
@RestController
@RequestMapping(value = PathContains.XUAT_CUU_TRO +PathContains.KIEM_TRA_CHAT_LUONG + PathContains.KIEM_NGHIEM_CHAT_LUONG)
@Slf4j
@Api(tags = "Xuất hàng DTQG - Xuất cứu trợ viện trợ - Phiếu kiểm nghiệm chất lượng")
public class XhCtvtPhieuKnClController extends BaseController {

  @Autowired
  XhCtvtPhieuKnClHdrService xhCtvtPhieuKnClHdrService;
  ...
  public ResponseEntity<BaseResponse> colection(@CurrentUser CustomUserDetails currentUser,
                                                @RequestBody SearchXhCtvtPhieuKnCl objReq) {
    BaseResponse resp = new BaseResponse();
    try {
      resp.setData(xhCtvtPhieuKnClHdrService.searchPage(currentUser,objReq));
  ...
  public ResponseEntity<BaseResponse> update(@CurrentUser CustomUserDetails currentUser, @Valid @RequestBody XhCtvtPhieuKnClHdrReq objReq) {
    BaseResponse resp = new BaseResponse();
    try {
      resp.setData(xhCtvtPhieuKnClHdrService.update(currentUser,objReq));
      resp.setStatusCode(EnumResponse.RESP_SUCC.getValue());
      resp.setMsg(EnumResponse.RESP_SUCC.getDescription());
    } catch (Exception e) {
      resp.setStatusCode(EnumResponse.RESP_FAIL.getValue());
  ...
  public ResponseEntity<BaseResponse> preview(@RequestBody HashMap<String, Object> body) {
    ...
    try {
      ...
      resp.setData(xhCtvtPhieuKnClHdrService.preview(body));
      resp.setStatusCode(EnumResponse.RESP_SUCC.getValue());
      resp.setMsg(EnumResponse.RESP_SUCC.getDescription());
    } catch (Exception e) {
      resp.setStatusCode(EnumResponse.RESP_FAIL.getValue());
