...
public class XhCtvtQdPdHdrController {
  ...
  @ResponseStatus(HttpStatus.OK)
  public ResponseEntity<BaseResponse> list(@CurrentUser CustomUserDetails currentUser, @RequestBody SearchXhCtvtQuyetDinhPdHdr objReq) {
    BaseResponse resp = new BaseResponse();
    try {
      resp.setData(xhCtvtQdPdHdrService.searchList(currentUser, objReq));
      resp.setStatusCode(EnumResponse.RESP_SUCC.getValue());
      resp.setMsg(EnumResponse.RESP_SUCC.getDescription());
    } catch (Exception e) {
      resp.setStatusCode(EnumResponse.RESP_FAIL.getValue());
